...
# 이게 원래 쓰던거(training ratio 3:1 안지킨거)
def train_gan_epoch1(generator, discriminator1, discriminator2, dataloader, optimizer_g, optimizer_d1, optimizer_d2,device):  # 한 에포크 학습 정의
    generator.train()
    discriminator1.train()
    discriminator2.train()
    epoch_g_loss, epoch_g1_l1_loss, epoch_g2_l1_loss, epoch_d1_loss, epoch_d2_loss = 0, 0, 0, 0, 0
    epoch_g1_adv_loss, epoch_g2_adv_loss = 0, 0
    progress_bar = tqdm(dataloader, desc="Training", leave=False)


    for inputs, gts, masks in progress_bar:  # 한 epoch 당 과정
        inputs, gts, masks = inputs.to(device), gts.to(device), masks.to(device)  # inputs = 4채널 concat된 이미지

        fake_images1, fake_images2 = generator(inputs, masks)

        # Train Discriminator1
        optimizer_d1.zero_grad()
        real_output1 = discriminator1(gts)  # gt를 d1에 넣은 결과(scalar)값
        fake_output1 = discriminator1(fake_images2.detach())  # gen2 결과를 d1에 넣은 결과(scalar)값
        ...
        masked_images = fake_images2 * masks
        masked_gt_images = gts * masks

        fake_output2 = discriminator2(masked_images.detach(), masks)
        real_output2 = discriminator2(masked_gt_images, masks)
        ...
        g1_adv_loss = -torch.mean(discriminator1(fake_images2))  # G1의 Unet_GAN Loss
        g2_l1_loss = nn.L1Loss()(fake_images2, gts)  # G2의 L1 Loss
        g2_adv_loss = -torch.mean(discriminator2(masked_images, masks))  # G2의 Unet_GAN Loss     # mean 쓰는 이유 batch 단위이므로 batch수에 대한 평균
        g_loss = g1_l1_loss + g2_l1_loss + g1_adv_loss + g2_adv_loss
        g_loss.backward()
        optimizer_g.step()

        epoch_g_loss += g_loss.item()
        epoch_g1_l1_loss += g1_l1_loss.item()
        epoch_g2_l1_loss += g2_l1_loss.item()
        epoch_g1_adv_loss += g1_adv_loss.item()
        epoch_g2_adv_loss += g2_adv_loss.item()
    ...
    return (epoch_g_loss / len(dataloader), epoch_g1_l1_loss / len(dataloader), epoch_g2_l1_loss / len(dataloader), epoch_g1_adv_loss / len(dataloader),
            epoch_g2_adv_loss / len(dataloader), epoch_d1_loss / len(dataloader), epoch_d2_loss / len(dataloader))
...
def train_gan_epoch(generator, discriminator1, discriminator2, dataloader, optimizer_g, optimizer_d1, optimizer_d2,device):  # 한 에포크 학습 정의
    ...
    for batch_idx, (images, gts, masks, large_masks) in enumerate(progress_bar):  # 한 epoch 당 과정
        ...
        g1_adv_loss = -torch.mean(discriminator1(fake_images2))  # G1의 Unet_GAN Loss
        g2_l1_loss = nn.L1Loss()(fake_images2, gts)  # G2의 L1 Loss
        g2_adv_loss = -torch.mean(discriminator2(fake_images2, large_masks))  # G2의 Unet_GAN Loss
        g_loss = g1_l1_loss + g2_l1_loss + g1_adv_loss + g2_adv_loss
        g_loss.backward()
        optimizer_g.step()
    ...
    return (epoch_g_loss / len(dataloader), epoch_g1_l1_loss / len(dataloader), epoch_g2_l1_loss / len(dataloader),
            epoch_g1_adv_loss / len(dataloader),
            epoch_g2_adv_loss / len(dataloader), epoch_d1_loss / (len(dataloader)//3), epoch_d2_loss / (len(dataloader)//3))
...
def validate_epoch(generator, discriminator1, discriminator2, dataloader, device, writer, epoch, save_dir=None):
    ...
    with torch.no_grad():
        for i, (images, gts, masks, large_masks) in enumerate(dataloader):  # 인덱스까지 얻기 위해 굳이 enumerate 사용 (i를 통해 5개만 저장하려고)
            ...
            val_g1_l1_loss += g1_l1_loss.item()
            val_g2_l1_loss += g2_l1_loss.item()
            val_g1_adv_loss += g1_adv_loss.item()
            val_g2_adv_loss += g2_adv_loss.item()
            val_d1_loss += d1_loss.item()
            val_d2_loss += d2_loss.item()
    ...
    val_g1_l1_loss /= len(dataloader)
    val_g2_l1_loss /= len(dataloader)
    val_g1_adv_loss /= len(dataloader)
    val_g2_adv_loss /= len(dataloader)
    val_d1_loss /= len(dataloader)
    val_d2_loss /= len(dataloader)
...
def main():
    save_dir = "D:/inpaint_result/CASIA_Distance/TT-GAN/db1_train_real_real"

    writer = SummaryWriter(os.path.join(save_dir, 'SR_Stage_4%s' % datetime.now().strftime("%Y%m%d-%H%M%S")))
    # Paths
    train_image_paths = 'C:/Users/8138/Desktop/DB/CASIAv4/CASIA_Ver4_Distance/reflection_random(50to1.7)_db1_224_trainset'  # List of input image paths
    train_mask_paths = 'D:/mask/CASIA_Distance/algorithm/450to50000_174x174padding_if_gac1_4000_algorithm/db1_test_layer28_0.3_only_mask_trainset'  # List of mask paths
    train_gt_paths = "C:/Users/8138/Desktop/DB/CASIAv4/CASIA_Ver4_Distance/db1_224_for_gt_inpainting_trainset"  # List of ground truth paths
    train_large_mask_dir = "D:/mask/CASIA_Distance/algorithm/450to50000_174x174padding_if_gac1_4000_algorithm/db1_test_layer28_0.3_only_mask_large_mask_h2_w2.2_trainset"  # List of large_mask paths

    val_image_paths = 'C:/Users/8138/Desktop/DB/CASIAv4/CASIA_Ver4_Distance/reflection_random(50to1.7)_db1_224_validset'  # List of input image paths
    val_mask_paths = 'D:/mask/CASIA_Distance/algorithm/450to50000_174x174padding_if_gac1_4000_algorithm/db1_test_layer28_0.3_only_mask_validset'  # List of mask paths
    val_gt_paths = "C:/Users/8138/Desktop/DB/CASIAv4/CASIA_Ver4_Distance/db1_224_for_gt_inpainting_validset"  # List of ground truth paths
    val_large_mask_dir = "D:/mask/CASIA_Distance/algorithm/450to50000_174x174padding_if_gac1_4000_algorithm/db1_test_layer28_0.3_only_mask_large_mask_h2_w2.2_validset"  # List of large_mask paths
    ...
    optimizer_d1 = optim.Adam(discriminator1.parameters(), lr=lr_d1, betas=(0.9, 0.999))
    optimizer_d2 = optim.Adam(discriminator2.parameters(), lr=lr_d2, betas=(0.9, 0.999))
